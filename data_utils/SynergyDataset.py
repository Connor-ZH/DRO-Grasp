...
import trimesh
import torch
from .dataset_2 import SynergyDatasetAllegroHandV2, SynergyDatasetBarrettV2, SynergyDatasetShadowHandV2
...
class SynergyDataset(Dataset):
    def __init__(
        ...
        for robot_name in self.robot_names:
            self.hands[robot_name] = create_hand_model(robot_name, torch.device('cpu'), dataset_type="SYNERGY")

        self.datasets = {}
        self.datasets['allegro'] = SynergyDatasetAllegroHandV2("train" if is_train else "test_unseen")
        self.datasets['shadow'] = SynergyDatasetShadowHandV2("train" if is_train else "test_unseen")
        self.datasets['barrett'] = SynergyDatasetBarrettV2("train" if is_train else "test_unseen")
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            for idx in range(self.batch_size):
                ...
                dataset = self.datasets[robot_name]
                trans, rot, object_pc, raw_articulation, object_name = random.choice(dataset)
                # print(f"[DEBUG] {robot_namee}, {object_name}, object_pc shape = {object_pc.shape}")

                
                indices = torch.randperm(2048)[:self.num_points]
                ...
                target_q = torch.cat([trans, rot, raw_articulation], dim=-1)
            ...
            for batch_idx in range(self.batch_size):
                ...
                # else:
                    # name = object_name.split('+')
                    # object_path = os.path.join(ROOT_DIR, f'data/PointCloud/object/{name[0]}/{name[1]}.pt')
                object_pc = dataset.pcl_dict[object_name]
                indices = torch.randperm(2048)[:self.num_points]
                object_pc = object_pc[indices]
    ...
    def __len__(self):
        if self.is_train:
            return math.ceil(
                (self.datasets["allegro"].__len__() +
                self.datasets["barrett"].__len__() +
                self.datasets["shadow"].__len__()) / self.batch_size
            )
        else:
