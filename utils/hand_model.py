...
def create_hand_model(
    ...
    if dataset_type == "DRO":
        ...
        links_pc_path = os.path.join(ROOT_DIR, f'data/PointCloud/robot_DRO/{robot_name}.pt')
    else:
        print(f"Building {robot_name} hand model for Synergy Dataset")
        urdf_path = f"data/synergy/robot_meta/{robot_name}/{robot_name}.urdf"
        meshes_path = f"data/synergy/robot_meta/{robot_name}/meshes"
        links_pc_path = os.path.join(ROOT_DIR, f'data/PointCloud/robot_Synergy/{robot_name}.pt')
